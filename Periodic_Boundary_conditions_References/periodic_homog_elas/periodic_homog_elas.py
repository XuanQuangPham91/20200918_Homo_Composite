...
# ## Introduction
# 
# This tour will show how to perform periodic homogenization of linear elastic materials.

# consisting of circular inclusions (numbered $1$) of radius $R$ with elastic properties $(E_r, \nu_r)$ and
# embedded in a matrix material (numbered $0$) of properties $(E_m, \nu_m)$ following an hexagonal pattern.
#
# A classical result of homogenization theory ensures that the resulting overall behavior will be isotropic,
# a property that will be numerically verified later.
...
# Because line magics are only supported by the IPython command line not by Python cl,
# use: 'exec(%matplotlib inline)' instead of %matplotlib inline
'exec(%matplotlib inline)'
# get_ipython().run_line_magic('matplotlib', 'inline') -> wrong syntax for IPython
# get_ipython().magic(u'matplotlib notebook') -> wrong syntax for IPython
...
# **Remark**: `mshr` does not allow to generate a meshed domain with perfectly matching vertices on opposite boundaries
# as would be required when imposing periodic boundary conditions. For this reason, we used a `Gmsh`-generated mesh.

# ## Periodic homogenization framework
# 
# The goal of homogenization theory consists in computing the apparent elastic moduli of the homogenized medium
# associated with a given microstructure. In a linear elastic setting, this amounts to solving the following
# auxiliary problem defined on the unit cell $\mathcal{A}$:
# 
...
# \end{equation}$$
# 
# where $\boldsymbol{E}$ is the **given** macroscopic strain, $\boldsymbol{v}$ a periodic fluctuation and
# $\mathbb{C}(\boldsymbol{y})$ is the heterogeneous elasticity tensor depending on
# the microscopic space variable $\boldsymbol{y}\in\mathcal{A}$. By construction, # the local microscopic strain is
# equal on average to the macroscopic strain: # $\langle \boldsymbol{\varepsilon} \rangle =
# \boldsymbol{E}$. Upon defining the macroscopic stress $\boldsymbol{\Sigma}$ as the microscopic stress average:
# $\langle \boldsymbol{\sigma} \rangle = \boldsymbol{\Sigma}$, there will be a linear relationship between
# the auxiliary problem loading parameters $\boldsymbol{E}$ and the resulting average stress:
# 
# $$\boldsymbol{\Sigma}  = \mathbb{C}^{hom}:\boldsymbol{E}$$
# 
# where $\mathbb{C}^{hom}$ represents the apparent elastic moduli of the homogenized medium.
# Hence, its components can be computed by solving elementary load cases corresponding to the different components of
# $\boldsymbol{E}$ and performing a unit cell average of the resulting microscopic stress components.
# 
# ### Total displacement as the main unknown
# 
# The previous problem can also be reformulated by using the total displacement $\boldsymbol{u}
# = \boldsymbol{E}\cdot\boldsymbol{y} + \boldsymbol{v}$ as the main unknown with now $\boldsymbol{\varepsilon}
# = \nabla^s \boldsymbol{u}$. The periodicity condition is therefore equivalent to the following constraint:
# 
# $$\boldsymbol{u}(\boldsymbol{y}^+)-\boldsymbol{u}(\boldsymbol{y}^-)
# = \boldsymbol{E}\cdot(\boldsymbol{y}^+-\boldsymbol{y}^-)$$
# 
# where $\boldsymbol{y}^{\pm}$ are opposite points on the unit cell boundary related by the periodicity condition.
# This formulation is widely used in solid mechanics FE software as it does not require specific change of
# the problem formulation but just adding tying constraints between some degrees of freedom.
# 
